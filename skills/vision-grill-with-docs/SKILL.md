...
스킬 호출 시 첫 단계는 **모드 분기**다. `grill_lib.py`의 `route_mode` 함수가 사용자 입력을 분석해 A/B/C 중 하나로 자동 분기한다.
