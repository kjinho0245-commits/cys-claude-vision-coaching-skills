...
박사님이 직접 정리하신 *비전 발견·완수 8단계* — 모든 스킬은 이 순서대로 사용한다.
...
│ ↓ │
│ 4. 마음을 사로잡는 가치 명확화 + 비전의 결과·열매 예상 │
│ ↓ │
│ 5. "가치 + 시대적 필요 + 내 능력" 종합 → 비전 출발점·큰 그림 │
│ ↓ │
│ 6. 비전선언문 작성 │
│ ↓ │
...
> 본 표는 `_build_catalog.py`가 자동 생성합니다. 새 스킬 추가 후 빌드 스크립트를 실행하면 갱신됩니다.
...
## 1️⃣ Vision Coding - 진단 7종 / Diagnosis (7 skills)

박사님 8단계 *1단계 — 내 안에 있는 것 파악*. CYS 비전 역량 검사를 중심으로 *내 안의 잠재력·성격·관심·재능·가치·준비도*를 다층 진단.
...
## 2️⃣ 박사님 책 토대 — 척추 도식 5종 / Spine (5 skills)

박사님 미래준비학교 *고유 프레임* — 다른 어디에도 없는 박사님 정통 도구. 8단계 *5·6·8단계*에 핵심.
...
## 3️⃣ 박사님 책 토대 — 살·근육 + 응용 7종 / Flesh + Applied (7 skills)

박사님 책의 *세부 도구* + 박사님 미래학 *응용 도구* — 8단계 *2·3·4·7단계*에 핵심.
...
## 4️⃣ 처방·실행 7종 / Prescription (7 skills)

일반 코칭 도구 (서양 코칭 자료와 결합) — 8단계 *5·7·8단계*에 핵심.
...
└─────────────────────────────────────────────────────────────────┘
 ↓
┌─────────────────────────────────────────────────────────────────┐
│ 5단계 — "가치 + 시대적 필요 + 내 능력" 종합 → 비전 출발점·큰 그림 │
│ ┌──────────────────┐ ┌──────────────────────┐ │
...
박사님 8단계 흐름 그대로 — 모든 26개 스킬 활용

### 🏫 고등학생 (High School) 진로 입문
...
### foresight-* 시리즈 (미래 예측)
- `foresight-futures-wheel` — 박사님 8단계 *2단계 (미래 예측)* 보강 도구
...
| 패턴 | 적용 |
|------|------|
| `*-visioncoding` | **검사 계열** (readiness·mbti·enneagram·strong·multipleintel·values·cys-competence 등 7개) |
| 자유 명명 | **처방·코칭·훈련 계열** (나머지 18개) |

박사님 메모리 [Vision visioncoding 명명 원칙](feedback_vision_visioncoding_naming.md) 참조.
